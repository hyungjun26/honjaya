...
def prep_display(dets_out, img, h, w, path, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        if args.skip != '[]':
            ...
            for i in skip:
                adj_index = int(i) - int(adj)
                masks_copy = torch.cat(
                    [masks_copy[:adj_index], masks_copy[adj_index+1:]])
                boxes = np.delete(boxes, adj_index, 0)
                scores = np.delete(scores, adj_index)
                classes = np.delete(classes, adj_index)
                num_dets_to_consider -= 1
        ...
        mask = sn.binary_dilation(mask).astype(mask.dtype)
        mask = sn.binary_dilation(mask).astype(mask.dtype)
        mask = sn.binary_dilation(mask).astype(mask.dtype)
        mask = sn.binary_dilation(mask).astype(mask.dtype)
        mask = sn.binary_dilation(mask).astype(mask.dtype)
        ...
        image_bgra = np.concatenate(
            [mask_image, np.full((h, w, 1), 255, dtype=np.uint8)], axis=-1)
        ...
        background2 = torch.mul(background.squeeze(
            0).permute(1, 2, 0).float(), 1 - mask)
...
def evalimage(net: Yolact, path: str, save_path: str = None):
    ...
    img_numpy = prep_display(preds, frame, None, None,
                             save_path, undo_transform=False)
...
def evaluate(net: Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        if ':' in args.image:
            a, inp, b, out = args.image.split(':')
            evalimage(net, a+':'+inp, b+':'+out)
